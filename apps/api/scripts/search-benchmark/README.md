...
Trois lectures du **même catalogue public figé** : SQL actuel, PostgreSQL enrichi, Elasticsearch enrichi. Les deux candidats partagent `model.ts` et `search-intent.ts`. Aucun de ces scripts n'est appelé par les routes publiques. Le [résultat daté](../../../../audits/2026-09-24/search-s1.md) distingue la qualité, le temps local et les limites restantes.
...
python3 -B apps/api/scripts/search-benchmark/evaluate.py /prive/documents.ndjson /prive/mesures.ndjson
```
...
```sh
npm run test -w @catwalks/api -- lib/search-intent.test.ts lib/search-benchmark.test.ts
