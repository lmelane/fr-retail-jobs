/**
 * ISO 3166-1 : code numérique (tel qu'écrit dans les ids de `world-atlas`)
 * → code alpha-2 (tel que `Job.country` est normalisé et que `lib/countries`
 * libelle). Dictionnaire embarqué, aucun réseau. Le Kosovo (XK, pas de code
 * numérique officiel) et deux entités sans id dans l'atlas (Chypre Nord,
 * Somaliland) restent sans correspondance : tracés en « sans offre ».
 */
...
/** Alpha-2 pour un id numérique d'atlas (« 250 » → « FR »), ou null. */
...
export const ANTARCTICA_NUMERIC = '010';

/** Tous les alpha-2 connus de la table (tests, couverture). */
export function knownAlpha2(): string[] {
  return Object.values(NUMERIC_TO_ALPHA2);
}
