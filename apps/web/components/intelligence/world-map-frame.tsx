...
import { windowAvailable } from '@/lib/intelligence/format';
 ...
 *
 * Le mode « nouvelles · 30 j » n'est proposé que si l'observation couvre la
 * fenêtre : sinon il colorierait le monde sur un compte égal au total (le
 * catalogue ayant été reconstruit, aucune offre n'a plus de 30 jours).
 */
export function WorldMapFrame({ children, legend }: { children: ReactNode; legend: ReactNode }) {
  ...
  const canShowNew = windowAvailable(30);
  return (
    <div>
      ...
      <svg className="wmap" data-mode={canShowNew ? mode : 'volume'} viewBox={`0 0 ${MAP_WIDTH} ${MAP_HEIGHT}`} role="img" aria-label="Carte du monde des offres actives, par pays">
        {children}
      </svg>
    </div>
  );
