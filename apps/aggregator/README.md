...
**5. Les 1 452 offres sans `postedAt` — qualifiées, aucune réparée.** Dossier complet : `audits/2026-09-09/lot4-world-coverage/ralph-lauren-postedat.md`. Réparties sur **10 sources**, qualifiées hors ligne sur le RAW archivé (`reference/undated-by-source.csv`), puis Ralph Lauren par **observation ciblée du board configuré** (première collecte de P2, éléments natifs archivés avec empreintes).

**Ralph Lauren (1 103) — absence réelle, pas une « limite éditeur » vague.** Les cartes publient *ville · référence · fonction* et **aucune date** ; les 6 fiches de détail lues n'ont **ni JSON-LD, ni `JobPosting`, ni `datePosted`**, et zéro occurrence de « posted »/« publish » — la seule date de la page est le badge « Great Place To Work AUG 2025-AUG 2026 ». Mais la liste porte un tri `data-sortBy="postedDate"` : **l'ATS détient la date, le tenant ne la publie sur aucune surface publique**. Piste du tri testée et close : les surfaces triées rendent des cartes et 0 date, sous aucun format. Portée assumée : 2 listes + 6 fiches établissent le **gabarit** du board (cohérent avec 1 103/1 104), pas chaque offre.

Les 349 autres : **41** Lindex (le RAW n'a que des dates de *vacance*, que l'adaptateur refuse délibérément de publier — pas un défaut) · **2** UNIQLO Workday (**défaut réel** : la date est dans le RAW, la colonne est vide ; cause **non établie**, donc **non réparée**) · **5** `element-6` (RAW vide) · **301** indécidables hors ligne, sur des sources datées à 84–99 % (résidu, pas gabarit). Deux erreurs de classement corrigées en route, même principe : *le sens d'un champ dépend de l'ATS, pas de son nom* — `date_start` d'EasyCruit n'est pas une date de publication, `jobPostingInfo.startDate` de Workday en est une.

**Aucune donnée réparée, aucune suppression, aucune désindexation.** Écrire une date absente l'inventerait ; publier une date de vacance mentirait sur le type ; réparer les 2 UNIQLO sans la cause les laisserait revenir au prochain run. Conséquence documentée et non masquée : ces offres restent **inéligibles Google Jobs** (1,8 % du catalogue), ce qui est préférable à des dates fausses.

**6. Le périmètre P2 initial, dossier par dossier** : `audits/2026-09-09/lot4-world-coverage/p2-perimetre.md` — résultat, preuve, traitement appliqué ou retenu, état exact et prochaine action pour chacun. Résumé : contradictions d'identité documentées **0** (mais 20 libellés en refus, à solder par la revue d'alias) · Talentsoft **cause corrigée et testée**, 102 offres à réparer (et non ~350 : la ville était déjà bonne, seul `location` était pollué) · Ulta **1** offre sous « Ulta Beauty, Inc. », fusion d'identité à appliquer · isFrance/countryCode **0 incohérence, dossier clos** · doublons **44 URLs / 47 offres en excès**, dont ~25 sur `externalId` instable · WTTJ **52** offres sous PAUSED, qualifiées sans réactiver les crons · FashionJobs **172** détachables sans effet + **585** dépendantes (583 FR) en attente d'arbitrage.
