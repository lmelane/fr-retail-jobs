...
No mutation. usage: read-crons.py [--json]"""
import json, datetime, sys
from railway_api import api as call
...
for name, sid in services.items():
    ...
    row = {'cronSchedule': si['cronSchedule'], 'startCommand': si['startCommand'], 'latestDeployment': si['latestDeployment'],
           'PIPELINE_PAUSED': vs.get('PIPELINE_PAUSED'), 'PIPELINE_CMD': vs.get('PIPELINE_CMD'), 'INGEST_ONLY_KEYS': vs.get('INGEST_ONLY_KEYS'), 'REFRESH_ONLY_KEYS': vs.get('REFRESH_ONLY_KEYS')}
    ...
    for key in ('INGEST_ONLY_KEYS', 'REFRESH_ONLY_KEYS'):
        if row[key]:
            out['problems'].append(f'{name}: leftover {key}={row[key]!r}')
