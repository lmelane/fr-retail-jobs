...
## Captures et rétention : lots 2 et 5A

Le [contrat maintenu](../../../../docs/architecture/native-capture.md) décrit les tables, les limites, la configuration privée et les commandes. [`raw-capture.mts`](raw-capture.mts) lit les réponses natives, les sorties par offre et les observations historiques, ou rejoue une extraction hors ligne. Le format 2 exige la consommation de toutes les réponses et la concordance des sorties ordonnées ainsi que des métadonnées complètes du résultat. Les collectes historiques sans manifeste ne peuvent pas produire cette validation. [`retention.mts`](retention.mts) prépare et applique un plan borné dont l’empreinte doit être fournie explicitement.
