...
def probe_stage(stage):
    ...
    for key in keys:
        ...
        proc = subprocess.run(['python3', 'apps/aggregator/scripts/coverage/probe-sources.py', stage['snapshot'], str(out), '--concurrency', '1', '--retry', '--keys', key], capture_output=True, text=True)
...
def research_stage(stage):
    ...
    with open(log, 'a') as fh:
        proc = subprocess.run(['npx', 'tsx', 'apps/aggregator/scripts/coverage/research-portals.mts', stage['input'], str(out), stage.get('mode', 'known')], stdout=fh, stderr=subprocess.STDOUT, text=True)
