 ...
 *
 * A held representation is one whose latest `job.publication_held` event has not been followed by an actual
 * publication of that representation — i.e. the posting is not active under that source.
 *
...
try {
  ...
  const held = rows.filter((r: any) => !r.published_now);
  ...
  const summary = {
    ...
    definition: 'A hold is a STATE: the latest hold event for a (source, externalId) that is NOT published today. Event counts are history and prove nothing about the current state.',
  ...
  const cols = ['sourceKey', 'external_id', 'reason', 'published_now', 'at'];
