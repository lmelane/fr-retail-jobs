...
try {
  ...
  // Cancel transport and wait for settlement; portal configuration stays stable.
  const result = await withSourceBudget(
    () => fetchAtsJobs(ats, config), deadlineMs, key,
    { softTimeoutMs: Math.max(1, deadlineMs - Math.min(30_000, Math.floor(deadlineMs / 10))) },
  );
