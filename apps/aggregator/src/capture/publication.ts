...
export async function readCapturedPublication(db: Prisma.TransactionClient, input: Input,
  ...
  if (!batch || batch.purpose !== 'JOBS' || batch.sourceKey !== input.sourceKey || batch.outcome?.status !== 'EXTRACTED') throw new Error('Adapter output refers to an invalid capture batch');
