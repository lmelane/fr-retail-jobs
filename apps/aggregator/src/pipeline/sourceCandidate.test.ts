...
it('previews registration without creating a source or revision',async()=>{
 const before=await p.sourceRevision.count();
 expect(await registerSourceCandidate(p,candidate,false)).toMatchObject({created:false,willCreate:true,source:null});
 expect(await p.source.count({where:{key:candidate.key}})).toBe(0);
 expect(await p.sourceRevision.count()).toBe(before);
});
it('includes URL patterns in candidate equality and never overwrites an existing registry',async()=>{
 const input={...candidate,jobUrlPattern:'https://example.test/jobs/{id}'};
 await registerSourceCandidate(p,input);
 await expect(registerSourceCandidate(p,{...input,jobUrlPattern:'https://example.test/other/{id}'},false)).rejects.toThrow('conflicts');
 expect((await p.source.findUniqueOrThrow({where:{key:candidate.key}})).jobUrlPattern).toBe(input.jobUrlPattern);
});
