...
it('keeps success and failure signals distinct and reports an unreachable monitor', async () => {
  vi.stubEnv('HEALTHCHECK_PING_URL', 'https://hc-ping.com/witness');
  const request = vi.fn().mockResolvedValue(new Response('', { status: 200 }));
  vi.stubGlobal('fetch', request);
  expect(await pingHeartbeat('start')).toBe('pinged');
  expect(request.mock.calls[0][0]).toBe('https://hc-ping.com/witness/start');
  request.mockClear();
  expect(await pingHeartbeat(true)).toBe('pinged');
  expect(request.mock.calls[0][0]).toBe('https://hc-ping.com/witness');
  expect(await pingHeartbeat(false)).toBe('pinged');
  expect(request.mock.calls[1][0]).toBe('https://hc-ping.com/witness/fail');
