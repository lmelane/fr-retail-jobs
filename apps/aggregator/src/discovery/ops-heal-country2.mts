...
const rows = await p.job.findMany({ where: { isActive: true, isFrance: true, country: null }, select: { id: true, location: true } });
...
for (const r of rows) {
  ...
  await p.job.update({ where: { id: r.id }, data: { country: 'FR' } });
  fixed++;
}
