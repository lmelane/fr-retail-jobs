...
/**
 * Audit a5 (liens morts) — exécuteur de SELECT en lecture seule.
 * Usage : npx tsx src/discovery/a5-sql.mts <fichier.sql>
 * Blocs séparés par `---`, nommés par `-- name: xxx`. Refuse tout ce qui
 * n'est pas SELECT/WITH.
 */
const file = process.argv[2];
if (!file) throw new Error('usage: a5-sql.mts <file.sql>');
