...
export async function planRetention(db: PrismaClient, keys: string[], now = new Date(), limit = 250): Promise<RetentionPlan> {
  ...
  const blobs = await db.rawBlob.findMany({ where: { body: { isNot: null },
    ...
    extractions: { none: { capturedAt: { gte: cutoff } } }, manifests: { none: { completedAt: { gte: cutoff } } }, completions: { none: { completedAt: { gte: cutoff } } },
    OR: [{ requestCaptures: { some: { batch: { sourceKey: { in: allowed } } } } }, { captures: { some: { batch: { sourceKey: { in: allowed } } } } }, { observations: { some: { sourceKey: { in: allowed } } } }, { extractions: { some: { batch: { sourceKey: { in: allowed } } } } }, { manifests: { some: { batch: { sourceKey: { in: allowed } } } } }, { completions: { some: { batch: { sourceKey: { in: allowed } } } } }],
...
export async function applyRetention(db: PrismaClient, plan: RetentionPlan, expectedHash: string, store: ObjectStore) {
  ...
  const scopedBlobs = await db.rawBlob.count({ where: { hash: { in: plan.blobs }, OR: [
    ...
    { completions: { some: { batch: { sourceKey: { in: plan.keys } } } } },
  ] } });
