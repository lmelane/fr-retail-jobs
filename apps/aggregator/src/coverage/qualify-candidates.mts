...
for (const dir of researchDirs) {
  ...
  for (const line of readFileSync(file, 'utf8').split('\n')) {
    ...
    for (const page of r.pages ?? []) for (const c of page.atsCandidates ?? []) {
      ...
      const entry = candidates.get(tenantKey) ?? { tenantKey, type: c.type, kind, config: c.config, careersUrl: c.careersUrl, actors: new Set(), labels: new Set(), proofs: [], confidence: c.confidence ?? 0 };
