...
export async function requireSourceValidation(db: Prisma.TransactionClient, revisionId: string, now = new Date()) {
  ...
  if (!validation || validation.verdict !== 'VALIDATED' || validation.captureBatch.purpose !== 'JOBS') throw new SourceValidationGateError('VALIDATION_MISSING', 'promote: no current validated native offer capture or proven empty feed');
