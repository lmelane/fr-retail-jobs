...
import { contexteTemoin, offreBrute, page } from './fixture.js';
...
describe.skipIf(!enabled)('re-projection du stock des offres directes (D-455)', () => {
  ...
  const ligneVersion1 = async (id: string, surcharges: Record<string, unknown>, ancien: string, eligible = true) => {
    ...
    const ligne = projeterOffreDirecte(offre, BigInt(7), BigInt(3), contexte);
  ...
  it('PRÉMISSE puis preuve — trois lignes de version 1 sont re-projetées depuis leur contrat, et rien d’autre ne change', async () => {
    ...
    await prisma.directOffer.create({ data: projeterOffreDirecte(lireOffre(offreBrute({ id: E, slug: `slug-${E}`, maison: null })), BigInt(8), BigInt(1), contexte) });
    await prisma.directOffer.create({ data: { ...projeterOffreDirecte(lireOffre(offreBrute({ id: F, slug: `slug-${F}` })), BigInt(9), BigInt(1), contexte), payload: {} } });
    ...
    expect(await reprojeterStock(prisma, contexte)).toEqual({ reprojetees: 3, nonReprojetees: [{ id: D, cause: 'ContratInvalideError' }] });
  ...
  it('idempotente : une seconde passe ne relit que l’illisible, qui reste signalé', async () => {
    expect(await reprojeterStock(prisma, contexte)).toEqual({ reprojetees: 0, nonReprojetees: [{ id: D, cause: 'ContratInvalideError' }] });
  });
  ...
  it('le consommateur du flux la lance avant de lire : un flux vide suffit à mettre le stock à jour', async () => {
    ...
    const stats = await consommerFlux(prisma, vide, { contexte });
  ...
  it('une version plus récente écrite entre la lecture et l’écriture n’est jamais écrasée par un contrat périmé', async () => {
    ...
    const p2 = projeterOffreDirecte(lireOffre(offreBrute({ id: G, slug: `slug-${G}`, maison: null, titre: 'Titre P2 plus récent' })), BigInt(20), BigInt(4), contexte);
    ...
    expect(await reprojeterLigne(prisma, lue, contexte)).toBe(0);
    ...
    await reprojeterStock(prisma, contexte);
  ...
  it('une ligne dont le contrat ne se projette plus ne bloque ni les autres lignes, ni la lecture du flux', async () => {
    ...
    expect(() => projeterOffreDirecte(refuse, BigInt(1), BigInt(1), contexte)).toThrow();
