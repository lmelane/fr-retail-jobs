...
import { classifyJob, classifyProgramType, BOOTSTRAP_TAXONOMY } from './taxonomy.js';
...
describe('classifyJob — tout en une passe', () => {
  it('assemble fonction, séniorité, code et dispositif, sans les dérivés du moteur (lot F1)', () => {
    ...
    expect(result.occupationStatus).toBe('CLASSIFIED');
    expect(result.programType).toBeNull();
    // Le groupe et les spécialisations se relisent depuis le manifeste : ils ne sont plus écrits.
    expect(Object.keys(result)).not.toEqual(expect.arrayContaining(['occupationGroup', 'occupationSpecializations', 'isRetail', 'isAiRelated', 'skills', 'taxonomyVersion']));
    expect(BOOTSTRAP_TAXONOMY.classify(result.occupationEvidence.inputTitle, 'Retail').occupationGroup).toBe('retail');
  });

  it('une fonction inconnue laisse la fonction nulle ; le corporate et l’atelier ne sont pas du retail', () => {
    expect(classifyJob({ title: 'Poste à pourvoir' }).jobFunction).toBeNull();
    for (const title of ['Financial Controller', 'Maroquinier']) {
      const decision = BOOTSTRAP_TAXONOMY.classify(title);
      expect(decision.jobFunction).not.toBeNull();
      expect(decision.occupationGroup).not.toBe('retail');
    }
  });
});
