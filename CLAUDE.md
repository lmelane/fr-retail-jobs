...
- [Reset du runtime Railway](docs/architecture/railway-runtime-reset.md) : GO conditionnel R1 → R5 reçu ; R1 PASS. R2 PASS : clone restauré, nouvelle API conforme, canari Oh My Cream COMPLETED sans erreur ni egress métier hors scope, RAW et `/emplois` validés, worker revenu sous pause. R3 autorisé par le GO conditionnel ; production encore inchangée. Corriger les erreurs locales et retester sans nouvelle autorisation ; arrêter uniquement sur un véritable blocage externe ou un écart de sécurité/données/configuration déployée.
