-- Lot F3 (2026-09-16) : preuve par redirection canonique. Le registre configure
-- souvent l'hôte vendeur d'un tenant (maison.teamtailor.com) que l'éditeur
-- redirige lui-même (301/302/307/308) vers son hôte canonique sous le domaine
-- officiel revu (careers.maison.example) : la Maison a délégué ce nom par son
-- DNS, et la collecte passe par lui. La politique d'inspection passe en version 3,
-- toujours avec la méthode OFFICIAL_DOMAIN et un témoin `document` ; l'ordinal
-- du témoin compte alors les sauts archivés (≥ 1) et le rapport porte l'hôte
-- canonique, qui doit être la page de preuve. Les politiques 1 et 2 restent
...
BEGIN
  ...
  IF NEW.verdict='VERIFIED' THEN
    -- Trois témoins admis : un lien HTML actif (a/iframe) de la page officielle vers le portail (OFFICIAL_LINK) ; le portail
...
BEGIN
  ...
  IF i.id IS DISTINCT FROM NEW."identityReviewId" OR i.sequence IS NULL OR i.verdict<>'VERIFIED' OR
    ...
    coalesce(i."relationReport"->>'policy','') NOT IN ('official-html-link/1','official-html-link/2','official-html-link/3') OR
    (i.method='OFFICIAL_DOMAIN' AND i."relationReport"->>'policy' NOT IN ('official-html-link/2','official-html-link/3')) OR
