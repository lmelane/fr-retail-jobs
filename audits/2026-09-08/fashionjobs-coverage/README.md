...
Le qualificatif « direct » dans ce rapprochement utilise le classement existant `Source.tier` (`EMPLOYER_DIRECT`, `ATS_OFFICIAL`, `GROUP_OFFICIAL`) et le statut ACTIVE. Ce classement doit lui-même être audité : un portail employeur officiellement lié sur WTTJ peut constituer une source valable tout en étant encore catalogué `SPECIALIST_JOBBOARD`. Les colonnes chiffrées décrivent ce classement actuel, sans décider de la qualité d’une source par le seul nom de son fournisseur. Une absence de lien direct dans ce tableau n’est donc pas une preuve d’absence de portail ou d’offres déjà couvertes.
...
Livraison vérifiée : PR #22 fusionnée sur `main` au commit `7990e14e82a0698ac76149cdba9c9a47b883daa1`. Les images des trois workers sont SUCCESS et `buildOnly` ; aucune ingestion supplémentaire n’a été déclenchée pour cette preuve. Le front conserve sa version précédente, inchangée par ce lot. [État exact](delivery.json), [déploiements](deployment-proof.json), [lecture de production après livraison](post-deploy-production-proof.json). CI : 1 362 tests unitaires, 196 d’intégration, 102 web réussis (2 conditionnels ignorés), 16 E2E réussis. Le témoin de parseur a été exécuté sur le HTML réel archivé ; il n’est pas présenté comme une exécution du parseur dans le service de production.
