...
try {
  const result = await p.$transaction(async tx => {
    ...
    return {
      ...
      validatedIdentityReviews: await tx.sourceIdentityReview.count({ where: { verdict: 'VERIFIED' } }),
    };
  }, { timeout: 60000 });
  writeFileSync('backups/remediation-20260908/directory-production-current.json', JSON.stringify(result), { mode: 0o600 });
  console.log(JSON.stringify({ at: result.at, companies: result.companies.length, sources: result.sources.length, validatedIdentityReviews: result.validatedIdentityReviews }));
