...
try{const proof=await p.$transaction(async t=>{await t.$executeRawUnsafe('SET TRANSACTION ISOLATION LEVEL REPEATABLE READ, READ ONLY');const batches=[];
 for(const snapshot of manifest.snapshots){
  ...
  for(const j of jobs){const before=snapshot.jobs.find((b:any)=>b.id===j.id),op=plan.operations.find((o:any)=>o.entity==='Job'&&o.id===j.id);
   if(!op || j.isFrance!==op.patch.isFrance || digest({...json(j),isFrance:before.isFrance,updatedAt:before.updatedAt})!==digest(before))throw new Error(`Unexpected Job change ${j.id}`);
  }
  for(const s of sources){const before=snapshot.sources.find((b:any)=>b.id===s.id),op=plan.operations.find((o:any)=>o.entity==='JobSource'&&o.id===s.id);
   if(op)throw new Error('Unexpected representation operation');
   if(digest(json(s))!==digest(before))throw new Error(`RAW or representation changed ${s.id}`);
  }
  ...
  batches.push({batchId:snapshot.batchId,jobs:jobs.length,representations:sources.length,correctedEvents:corrections.length,ledger:ledger.length,commits:[...new Set(ledger.map(e=>e.commitHash))],rawPreserved:true,originalHistoryPreserved:true,otherJobFieldsPreserved:true,geographyUnchanged:true});
 }
 return{at:new Date().toISOString(),evidenceHash:manifest.evidenceHash,batches,counts:await t.$queryRaw`SELECT count(*)::int AS active,count(*) FILTER(WHERE "postedAt" IS NULL)::int AS "withoutPublicationDate",count(*) FILTER(WHERE "isFrance")::int AS france,count(*) FILTER(WHERE "countryCode"='FR')::int AS canonicalFrance,count(*) FILTER(WHERE "countryCode" IS NOT NULL AND "isFrance" IS DISTINCT FROM ("countryCode"='FR'))::int AS contradictions FROM "Job" WHERE "isActive"`};
