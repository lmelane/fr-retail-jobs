...
- `families-bounded-run.md` + `families/` — run borné en production des familles corrigées (Rituals prouvé 1 123/1 123 ; Kering, Mango, Capri, Nordstrom, Swatch) ; constat systémique : 178 nouvelles annonces refusées par la porte d'identité sur des libellés d'entités juridiques (sources non certifiées).
- `legal-entity-aliases.md` + `aliases-sitemaps/` — 15 alias source-scopés dérivés de décisions existantes (clé canonique = marque ; référentiel D45) qui débloquent la création d'offres sur Nordstrom, Mango, Capri ; 5 sources génériques passées au sitemap d'éditeur (Oniverse 483 offres au lieu de 20) ; crons gelés (D57).
- `proof-dimensions.md` + `tracker-v7/` — les cinq dimensions de preuve séparées (identité 13 · énumération 385 · collecte complète 377 · détails 243 · ingestion 162 · visibilité 1) ; fiches `refused-labels-review.csv` (83 libellés refusés, classés) et `candidates-prescreen.csv` (152 tenants).
- `decisions-applied-evening.md` + `qualification-evening/` — décisions du cadrage du soir appliquées sur preuves archivées : URBN sous-libellés distincts, règle Kering (5/6 hors ligne), périmètre certifié Workday, audit des alias, Aptar mesuré, alias v2 (27), rejeu hors ligne de 410 charges.
- `final-validation-controls.md` — les seuls contrôles qui exigent une lecture actuelle ou une ingestion bornée (A1–A6, B1–B6, C1–C3) et les décisions en attente (D).

- [Qualification du 2026-09-10](qualification-2026-09-10.md) — contrôles A1–A5 rejoués (407/407 reçus courants, 0 déficit non expliqué), Mango `SINGLE_BRAND` certifié par vérification, Kering Corporate → Kering (preuve API + kering.com), Reclectic qualifié (pages URBN), 36 alias v3 sur preuves, 6 sources retirées (tenants erronés, doublon Browns), portails Shiseido / Groupe Rocher rendus au groupe, fiche Aptar par offre, 212 marques / 152 tenants, tableau par source (`tracker-v8/`), preuves `qualification-0910/`.
