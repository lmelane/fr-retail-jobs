...
**5A à 5F validés localement : [manifeste intégral et rejeu strict](lot-5a.md), [configuration stable et budgets séparés](lot-5b.md), [révisions immuables du registre](lot-5c.md), [validation technique native et porte de promotion](lot-5d.md), [revues d’identité liées à la révision](lot-5e.md), puis [parcours unique des sources et retrait des anciennes orchestrations](lot-5f.md). Les sous-lots 5G (identité, accès, admission, frontière de publication, fin d’ingestion et absence) sont validés localement jusqu’à 5G3C ; les rôles de source, les familles non qualifiées et la re-qualification du stock restent à terminer.**
...
- **5G3B2** : remplacer les champs mutables d’accès par une décision immuable portant sur les requêtes et l’identité de collecteur réellement utilisées. [Bilan](lot-5g3b2.md). Les rôles employeur, groupe et éditeur restent à compléter au-delà de leur séparation dans la résolution des offres.
- **5G3B3A / 5G3B3B** : admission immuable avant réseau pour chaque ingestion, capture admise obligatoire pour publier et retirer, décisions de périmètre hors du RAW. [Bilans](lot-5g3b3a.md) et [5G3B3B](lot-5g3b3b.md).
- **5G3C** : fin d’ingestion immuable (`SourceIngestionCompletion`) et preuves d’absence fondées sur la capture admise, scellée et achevée, sous la porte de publication courante ; manifeste de refresh version 4 ; suppression du nettoyage de génération, de l’ancien témoin de test fondé sur `SourceRun`/`PipelineEvent` et de deux scripts lisant ce journal ; contrat canonique Ashby. [Bilan](lot-5g3c.md). Reste : déclarer le contrat canonique dans les 41 autres familles et re-qualifier chaque source avant qu’une absence puisse être prouvée sur le stock.
- **F6, 17/09/2026** : contrat canonique étendu de 5 à 28 familles sur 44, 16 refusées avec motif établi. [Bilan](lot-f6-contrats-canoniques.md). Reste : six familles déclarent un contrat **inerte**, faute de terminaison probante (relevé `DECLARED_BUT_NOT_PROVING` dans `pipeline/refreshPlan.ts`) — leur promotion est un arbitrage sur ce qui fait preuve, à soumettre. La re-qualification et la ré-ingestion admise de chaque source restent nécessaires : déclarer le contrat ne ferme rien par lui-même.
