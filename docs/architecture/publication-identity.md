...
La répartition version 5 conserve normalement l'ancien ID public sur la publication native qui l'identifie. Lorsqu'elle n'est pas qualifiée, `withdrawJobIds` permet de préserver cet ID comme fiche retirée, sans lui affecter une autre candidature. Son propriétaire natif doit figurer dans `quarantineSourceIds`, l'ID ne peut être repris par aucun groupe résultant et une redirection existante est refusée. Une identité native explicite prime sur la simple égalité d'URL. Une fermeture déjà prouvée exige une revue de cycle de vie distincte ; elle n'est ni effacée ni transformée en retrait.

Le retrait conserve le contenu historique en stockage privé, met la Job inactive avec `withdrawalReason=PUBLICATION_UNVERIFIED`, puis journalise `WITHDRAWN`. Le groupe peut perdre toutes ses publications. Une nouvelle collecte qualifiée ne récupère pas silencieusement cet ancien ID. Le [lot 4H3](../../audits/reprise-2026-09-15/lot-4h3.md) vérifie le cas concret et son parcours public.

L'API distingue `active`, `closed`, `withdrawn` et `missing`. Un retrait du catalogue n'affirme jamais que l'employeur a fermé son recrutement. La fiche retirée sans contenu fiable répond HTTP 410 avec son ID, `job: null`, sans URL de candidature ni redirection supposée. Le site montre un état de retrait et un retour à la recherche, même si cette recherche est vide ou indisponible. Le retour conserve les critères et le marché explicite. Les pages HTML restent `noindex` ; leur statut HTTP public définitif fait partie de la bascule SEO.
...
Pour iCIMS, la page retenue, son identifiant numérique, la référence native lorsqu'elle existe et l'URL déclarée par son unique `JobPosting` doivent concorder. Deux flux se rapprochent seulement sur la même origine régionale et le même identifiant. Les paramètres qualifiés `hub` et `in_iframe` ne changent pas cette identité ; les URLs enregistrées restent intactes. Un hub peut nommer au maximum 32 origines HTTPS natives exactes dans `config.detailOrigins`, après revue des liens capturés. Cette autorisation de lecture ne fusionne pas les tenants régionaux. Le collecteur vérifie le périmètre avant de lire un détail et met en attente les pages absentes, ambiguës ou contradictoires. La même liaison est contrôlée lors de la reprise historique ; voir le [sous-lot 4H1](../../audits/reprise-2026-09-15/lot-4h1.md).

`clusterKey` est une clé de recherche indexée calculée à partir de l’identité de candidature qualifiée, ou du couple source / identifiant. Ce n’est plus une clé employeur / ville. Une correction d’employeur ne réécrit pas cette clé. Les anciennes clés nécessitent une reprise dédiée avant la bascule. Le format persistant d’une clé est séparé de la version du lecteur de preuve : l’ajout de Workday ne renumérote pas les clés Oracle ou JobAffinity. Workday utilise un espace de clés de réquisition propre ; aucune réquisition n’est déduite du slug ou d’un suffixe numérique. La clé Teamtailor associe l’émetteur déclaré, l’identifiant numérique et le UUID natif. Une publication sans preuve d’équivalence conserve sa clé propre à la source ; elle n’est pas fermée pour cette raison.
