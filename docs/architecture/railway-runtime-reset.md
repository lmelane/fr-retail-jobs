...
**État courant : [bilan post-RUN](../../audits/2026-09-23/post-run.md).** Deux runtimes, PostgreSQL conservé. Exploitation : un RUN quotidien à 18 h Europe/Paris, chargé et attesté ; correctifs du RUN initial validés par replays ciblés. Les sections R1–R5 ci-dessous documentent la transition déjà exécutée, pas un protocole à recommencer.
...
Lors de R5, le domaine `agregator.catwalks.io` a basculé vers la nouvelle API, déploiement `0d62ade6-efc0-45cc-a3a6-1c69a0864199`, image `c3613a1`. Le CNAME `4b8h83cv.up.railway.app` est propagé, DNS seul et TTL automatique conservés, certificat valide. Sauvegarde privée du CNAME précédent `4j23i5c2.up.railway.app` et du rattachement Railway dans le dossier d'exploitation. Health/auth, FR (4 440), US (18 287), filtre Maison (Oh My Cream : 21), fiche et rendu du site local relié au domaine public : PASS. Aucun rollback, aucune ingestion, aucun changement de code pendant R5.

**Clôture autorisée après R5 :** les quatre anciens services (`catwalks-aggregator`, `catwalks-refresh`, `catwalks-reconcile`, `catwalks-api`) sont supprimés. L'ancienne API n'était plus appelée par `/offres` ou le matching ; leurs chemins Catwalks backend sont conservés. La DB et son volume sont intacts. Le GO suivant a autorisé le worker générique, le renouvellement de ses preuves d’accès et le run complet des ACTIVE ; le calendrier normal est maintenant actif après clôture sans défaut systémique. Les reçus R1–R5 restent les preuves historiques, sans nouvelle répétition.

**Preuve historique R1 : PASS après accord de Loïc sur les images publiques.** Les deux images initiales étaient `c3613a1` ; le worker est ensuite passé par `653920c`. Les images post-RUN remplacent ces versions sur les deux services. Le [reçu de release courant](../operations/railway/runtime-release.json) fixe chaque digest, son contrat embarqué et les réglages d’exploitation résolus. Voir le [reçu historique R1](../../audits/2026-09-23/runtime-reset-r1.md).
...
Les sections 1 et 5 retracent le reset déjà réalisé. Les profils sous pause restent la configuration de repli. Le calendrier et la pause de production courants sont ceux du bilan en tête et du reçu de release ; ils ne nécessitent aucun rebuild d’image.
...
| `catwalks-ingestion-worker` | `sh apps/aggregator/start.sh scheduled`, entrée durcie | Normal : `PIPELINE_PAUSED=0`, 18 h Europe/Paris ; repli : `1`, CRON absent | événement de pause, signaux de run et heartbeat indépendant ; `NEVER` | PostgreSQL privé, Healthchecks, Brevo |
...
| Worker | `NODE_ENV=production`, `PIPELINE_PAUSED=0` en exploitation / `1` en pause, `BREVO_SENDER_NAME=Catwalks`, `CATWALKS_RUNTIME_PROFILE` | `DATABASE_URL`, `HEALTHCHECK_PING_URL`, `BREVO_API_KEY` ; références privées explicites `BREVO_SENDER_EMAIL`, `ALERT_EMAIL` |

### Troisième service, défini et non activé : `catwalks-direct-sync` (D-444)

Sur `development` (25/09/2026), le [contrat d’exécution](../operations/railway/runtime-target.json) définit un troisième service, qui n’existe pas sur Railway : il tourne sur l’image même du worker (`imageOf: catwalks-ingestion-worker`, même digest, aucun paquet supplémentaire), avec la commande `sh apps/aggregator/start.sh direct-liste`, CRON `*/5 * * * *`, une réplique, `NEVER`, aucun domaine. Il s’atteste sous le rôle `direct-sync` : la seule commande admise est `direct-liste` (`directSyncArguments`, `packages/runtime/index.mjs`) ; le worker ne peut pas la lancer et ce service ne peut pas lancer une ingestion (nom de service vérifié). Variables : `NODE_ENV=production`, `PIPELINE_PAUSED=1` par défaut (`0` à l’activation), `CATALOGUE_LISTE_URL=https://catwalks.api.catwalks.io` fixée et attestée ; secrets `DATABASE_URL` et `HEALTHCHECK_PING_URL` d’un check Healthchecks dédié (période 5 minutes), jamais celui du worker ; aucune clé du backend ni variable `CATALOGUE_FLUX_*`. `verify-image.mjs` est configuré pour démarrer aussi l’image du worker sous ce service, hors réseau, en pause, avec sa commande (`sharedOfflineEntrypoints` du reçu) ; ce chemin n’a pas encore tourné (aucun commit `[runtime-images]` de ce lot, aucune image construite localement). Le démarrage en pause de la commande est prouvé hors image (`pipeline-pause.test.mjs`), et l'attestation du rôle par `packages/runtime/test.mjs`. Sa pause est sa propre variable `PIPELINE_PAUSED` : mettre le worker en pause ne l’arrête pas, et l’inverse. Chaque passe vérifie le schéma (`prisma migrate status`, `src/worker.ts`) avant de lire la liste : une migration s’applique avant l’image qui l’attend, et le service se redéploie sur le digest du worker à chaque release de celui-ci (aucun contrôle ne l’impose). Sa création, son check et sa mise en marche sont des gestes de production sur GO explicite, jamais pendant le RUN de 18 h.
...
**Historique du reset R1 :** Recréer des services à `72300c9` ne suffit pas à retirer la sonde, le deadman et le défaut de commande. La production est restée à ce SHA jusqu’à validation de la candidate du reset. Construire une fois les deux images, enregistrer les digests et promouvoir exactement les images testées sur clone. Accès au registre OCI et authentification de pull Railway à vérifier avant provisioning ; pas de rebuild silencieux à la bascule.
...
## 5. Séquence historique du reset et critères d’arrêt
...
La clôture requiert un rapport court : diff cible/effectif/processus nul hors valeurs secrètes et identifiants générés classifiés, canari PASS, suivi existant attesté, services retirés listés, PostgreSQL/volume inchangés et reprise du ramp-up possible. Ces preuves sont acquises et référencées en tête ; les étapes historiques ne sont pas à rejouer sans changement qui les invalide.

### Suivi du canari — décision du propriétaire

Aucun nouveau dashboard, outil de surveillance ou moniteur. Utiliser les logs live Railway du service et du digest attendus, le module Healthchecks existant (`/start`, succès ou `/fail`), les événements `run.alive`/statuts DB et les captures RAW, puis `/emplois`. Le retour du worker sous pause est contrôlé séparément. Pour chaque image, utiliser son propre contrat embarqué et le reçu `runtime-release.json` : les deux services post-RUN utilisent le même contrat générique, chacun avec son profil propre. Le reçu résout explicitement la pause et le CRON autorisés en production. Ne pas appliquer le contrat de l’une à l’autre ni reconstruire une image pour une simple mise à jour documentaire.

## Exploitation quotidienne après l’audit post-RUN

Railway planifie ses CRON en UTC et peut décaler leur départ de quelques minutes ([documentation Railway](https://docs.railway.com/cron-jobs)). Le service doit terminer pour permettre le départ suivant.
