...
**Preuve historique R1 : PASS après accord de Loïc sur les images publiques.** Les deux images initiales étaient `c3613a1` ; l’API conserve son image, tandis que le worker utilise désormais `653920c`. Le [reçu de release courant](../operations/railway/runtime-release.json) fixe chaque digest, son contrat embarqué et les réglages d’exploitation résolus. Voir le [reçu historique R1](../../audits/2026-09-23/runtime-reset-r1.md).
...
| `catwalks-ingestion-worker` | `sh apps/aggregator/start.sh`, entrée durcie | Normal : `PIPELINE_PAUSED=0`, toutes les 4 h UTC ; repli : `1`, CRON absent | événement de pause, signaux de run et heartbeat indépendant ; `NEVER` | PostgreSQL privé, Healthchecks, Brevo |
