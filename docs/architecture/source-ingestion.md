...
Contrat vérifié dans le code le 22 septembre 2026, après F5. Ce contrat couvre les collectes, les écritures d’ingestion, la fin d’ingestion et les preuves d’absence qui en découlent. Il complète la [qualification](source-onboarding.md) et le [contrat d’accès](source-access.md).
...
Le choix ingestion/qualification et la révision demandée sont copiés et figés avant la première attente asynchrone ; une mutation de l’objet appelant ne peut pas les remplacer. Le batch et sa ligne `SourceIngestionAdmission` sont créés dans la même transaction. Cette ligne conserve la qualification ayant justifié le démarrage ; `identityReviewId` est facultatif et vaut `null` dans les nouvelles admissions. Le batch porte la révision du registre et l’identifiant de décision d’accès. Depuis F5, la revue d’identité par capture reste disponible mais ne conditionne plus la promotion ni l’admission. Elle est immuable. Une absence de décision obligatoire ou un refus annule l’allocation avant tout transport.

SQL exige une source ACTIVE, sa révision courante, la décision d’accès et la validation native courantes, leurs politiques et leur fraîcheur. Le batch doit avoir été créé dans la transaction courante et ne doit contenir aucun reçu ou résultat. Une sonde antérieure ne peut donc pas devenir rétroactivement une ingestion admise. La migration ne fabrique aucune admission historique.
...
Une nouvelle revue d’identité seule n’invalide plus la collecte. Une modification de la révision du registre ou de la décision d’accès impose une nouvelle collecte. Une validation plus ancienne ne qualifie pas les nouveaux octets. Les requêtes déjà parties restent conservées ; une révocation ne peut les rappeler rétroactivement.
...
Les adaptateurs qui n’archivent pas d’identifiants canoniques restent incapables de prouver une absence. L’éligibilité se mesure source par source sur la base visée ; un constat historique ne décrit pas une nouvelle collecte. Une fermeture par absence reste distincte d’une échéance native dépassée, d’un retrait natif capturé et d’un retrait administratif.
...
Les demandes de certification d’identité sans contrat prouvé restent refusées ; elles sont distinctes du droit d’ingérer depuis F5. Ce document ne certifie ni l’ensemble des sources existantes ni une release Railway différente du lecteur effectivement testé. Le CRON reste une phase distincte.
